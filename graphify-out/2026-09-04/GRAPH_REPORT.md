...
## Corpus Check
- 92 files · ~463,144 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 523 nodes · 686 edges · 41 communities
...
8. `mapLinear()` - 9 edges
9. `BtnCap()` - 9 edges
10. `Sentinel1SARDataset` - 9 edges
...
Cohesion: 0.04
Nodes (45): 10. SELF-AUDIT DECLARATION, 1. `data/currents/demo_currents.nc` (2.75 MB), 1. ENVIRONMENT & FORENSIC BASELINE (STEP 0), 2. `data/wind/demo_wind.nc` (446 KB), 3.1 Raster File Integrity & Metadata, 3. PHASE 2 — SAR SCENE AUTHENTICITY AUDIT, 4.1 Production Pipeline Execution, 4.2 Critical Finding: 4-Class Segmentation Claim is REFUTED (+37 more)
...
### Community 4 - "generate_demo_cache.py"
Cohesion: 0.08
Nodes (20): get_detector(), Preprocessing: calibration, speckle filter, land mask, geocode, SpillDetector, UNet, DriftSimulator, get_drifter(), Initialize an OpenOil model and attach readers., Physical advection kinematics fallback using verified CMEMS current vectors… (+12 more)
...
### Community 10 - "2.3 Training Data Forensics (`notebooks/train_unet.ipynb` & `models/train_unet.py`)"
Cohesion: 0.12
Nodes (17): 2.1 State Dict & Tensor Counts, 2.2 Architecture Comparison & Weight Loading Mechanism, 2.3 Training Data Forensics (`notebooks/train_unet.ipynb` & `models/train_unet.py`), 2.4 Git History & Training Logs Audit, 2. PHASE 1 — WEIGHTS & ARCHITECTURE FORENSICS, Command, Command, Command (+9 more)
...
- **Why does `SWACHH TRACK // ADVERSARIAL MODEL & CLAIMS AUDIT REPORT` connect `SWACHH TRACK // ADVERSARIAL MODEL & CLAIMS AUDIT REPORT` to `2.3 Training Data Forensics (`notebooks/train_unet.ipynb` & `models/train_unet.py`)`?**
  _High betweenness centrality (0.012) - this node is a cross-community bridge._
- **Why does `PROJECT CONTEXT: SWACHH TRACK — MARINE OIL SPILL DETECTION & VESSEL ATTRIBUTION SYSTEM` connect `PROJECT CONTEXT: SWACHH TRACK — MARINE OIL SPILL DETECTION & VESSEL ATTRIBUTION SYSTEM` to `10. Key Code Snippets`, `11. How to Run`, `6. Backend Services & Endpoint Specifications`, `7. Machine Learning Pipeline & Hydrodynamic Simulation`, `5. Frontend Architecture & API Consumption`, `8. Data Inventory & Specifications`, `9. Benchmarks & Evaluation Results`, `12. Current State & Codebase Health`?**
